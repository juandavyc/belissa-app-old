/* filechooser.camera.js v4 | @juandavyc | MIT licensed */
/* require modal */

class FileChooserCamera {

    constructor({ folder, rotate, url }) {
            ...
            content: () => {
                return $.ajax({
                    ...
                    if (response.statusText === 'bien') {
                        ...
                        _class.inputControlValue.value = response.src;
                        const element = document.getElementById(_class.cameraOptions.inputId + '-src');
                        if (element !== null) {
    ...
    uploadPhoto() {
        ...
        let self = $.confirm({
            ...
            content: () => {
                return $.ajax({
                    url: _class.urlUpload.photo,
                    type: 'POST',
                    data: {
                        image_base64: _class.cameraCanvas.toDataURL(),
                        image_folder: _class.cameraOptions.dataFolder,
                        image_rotate: 0,
                    },
                    headers: {
                        'csrf-token': $('meta[name="csrf-token"]').attr('content'),
