...
if (
    ...
    if ($this->arrayResponse["statusText"] != 'sin_resultados') {
        $conductorData = new ClienteClass($this->pdo);
        $this->arrayResponse['vehiculo'][0]["conductor"] = $conductorData->getInformacion(
            'ID',
            $this->arrayResponse['vehiculo'][0]["conductor"]["id"],
        );


        //llamar propietario
        $propietarioData = new ClienteClass($this->pdo);
        $this->arrayResponse['vehiculo'][0]["propietario"] = $propietarioData->getInformacion(
            'ID',
            $this->arrayResponse['vehiculo'][0]["propietario"]["id"],
        );
    }
    //llamar conductor
