...
require $_SERVER["DOCUMENT_ROOT"] . '/modulos/app/clases/editar_usuario/editar_usuario.class.php';
// cambiar la comprobacion de la sesion para devolver el error
$config = new RecursosApp();
$method = isset($_GET['m']) ? $_GET['m'] . 'editarusuario' : 'error';
...
if (
    ...
    if ($database->estadoConexion()['status']) {
        ...
        $editarusuarioModelo = new EditarUsuarioModelo($database->getPDO(), $config);
        // var_dump($method);
        $editarusuarioModelo->$method();
    } else {
...
class EditarUsuarioModelo
{
    ...
    public function Createeditarusuario()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/editar_usuario/editar_usuario.create.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }


    public function Infoeditarusuario()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/editar_usuario/editar_usuario.informacion.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }
}
