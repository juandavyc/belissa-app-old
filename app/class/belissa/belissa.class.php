...
class BelissaLogClass
{
    ...
    public function ListadoBelissa(
        $_condicional_ = array(
            'ORDER' => 'bel.id',
            'BY' => 'DESC',
            'PAGE' => '1',
        ...
        try {
            ...
            $mysqlQuery .= "bel_tip.id LIKE :tipo AND bel_mod.id LIKE :modulo ";
    ...
    public function infoBelissa($_id = 0)
    {
        ...
        try {
            $mysqlQuery = 'SELECT 
                bel.descripcion,
                bel_tip.nombre AS tipo_bel,
                bel_mod.nombre AS nombre_mod,
                usu.nombre AS usuario_nombre,
                bel.fecha_formulario
            FROM
                belissa_log bel
                    INNER JOIN
                belissa_tipo bel_tip ON bel_tip.id = bel.id_belissa_tipo
                    INNER JOIN
                belissa_modulo bel_mod ON bel_mod.id = bel.id_belissa_modulo
                    INNER JOIN
                usuario usu ON usu.id = bel.id_usuario
            WHERE
                bel.id = :id;';
