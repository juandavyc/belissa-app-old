...
class AgendamientoClass
{
    ...
    public function gestionarAgendamiento(
        ...
        try {
            ...
            $mysqlQuery = "CALL proc_agendamiento_cda(";
            ...
            $mysqlStmt->bindParam(':id_estado', $_id_estado, PDO::PARAM_INT);
            ...
            $mysqlStmt->bindParam(':telefono', $_telefono, PDO::PARAM_STR);
            $mysqlStmt->bindParam(':correo', $_correo, PDO::PARAM_STR);
            $mysqlStmt->bindParam(':canal', $_canal, PDO::PARAM_INT);
    ...
    public function getBuscador(
        ...
        try {
            ...
            #Columnas
            $mysqlQueryColumns .= "agen.id, veh.placa, tveh.nombre AS nombre_tipoveh, ";
            $mysqlQueryColumns .= "agen.fecha, hor.nombre AS horario, ";
            $mysqlQueryColumns .= "can.nombre AS canal,";
            $mysqlQueryColumns .= "esagen.id AS id_estado, esagen.nombre AS nombre_estado, ";
            ...
            $mysqlQuery .= "LEFT JOIN tipo_vehiculo tveh ON veh.id_tipo_vehiculo = tveh.id ";
            ...
            $mysqlQuery .= "AND can.id LIKE :canal ";
            $mysqlQuery .= "AND tveh.id LIKE :tipoveh ";
            $mysqlQuery .= "AND hor.id LIKE :horari ";
            ...
            $mysqlStmt->bindParam(':tipoveh', $_condicional_['TIPO_VEHICULO'], PDO::PARAM_INT);
            $mysqlStmt->bindParam(':horari', $_condicional_['HORARIO'], PDO::PARAM_INT);
                        ...
                        while ($row = $mysqlStmt->fetch(PDO::FETCH_BOTH)) {
                            ...
                            array_push(
                                ...
                                array(
                                    ...
                                    "tipo_vehiculo" => htmlspecialchars($row['nombre_tipoveh']),
    ...
    public function getInformacion(
        ...
        try {
            ...
            $mysqlQuery .= "veh.id AS id_vehiculo, veh.placa AS placa_vehiculo,veh.id_tipo_vehiculo, ";
            $mysqlQuery .= "cli.id AS cliente_id, cli.nombre AS nombre_cliente, cli.apellido AS apellido_cliente,";
            $mysqlQuery .= "cli.telefono_2,cli.email,cli.id_tipo_documento,cli.documento, ";
    ...
    public function EditAgendamiento(
        ...
        try {
            $mysqlQuery = "UPDATE agendamiento ";
            $mysqlQuery .= "SET fecha = :fecha, id_horario = :horario, id_estado_agendamiento = :estado_agendamiento, ";
            ...
            $mysqlStmt->bindParam(':estado_agendamiento', $estado_agendamiento, PDO::PARAM_STR);

            if ($mysqlStmt->execute()) {
                $this->arrayResponse = array(
                    'statusCode' => 200,
                    'statusText' => 'bien',
                    'message' => 'El agendamiento actualizado ',
                );
    ...
    public function getAgendamientRtm(
        $_column = 'ID',
        $_id_vehiculo = 1,
        $_limite = '0,5'
    ) {
        ...
        try {
            $mysqlQuery = "SELECT ";
            $mysqlQuery .= "agen.id,agen.fecha,agen.fecha_formulario, ";
            $mysqlQuery .= "hora.nombre As nombre_horario, esta.nombre As nombre_estado, ";
            $mysqlQuery .= "can.nombre AS nombre_canal, ";
            $mysqlQuery .= "CONCAT(usu.nombre,' ',usu.apellido) As responsable ";
            $mysqlQuery .= "FROM ";
            $mysqlQuery .= "agendamiento agen ";
            $mysqlQuery .= "LEFT JOIN vehiculo veh ON agen.id_vehiculo = veh.id ";
            $mysqlQuery .= "LEFT JOIN horario hora ON agen.id_horario = hora.id ";
            $mysqlQuery .= "LEFT JOIN estado_agendamiento esta ON agen.id_estado_agendamiento = esta.id ";
            $mysqlQuery .= "LEFT JOIN canal can ON agen.id_canal = can.id ";
            $mysqlQuery .= "LEFT JOIN usuario usu ON agen.id_usuario = usu.id ";
            $mysqlQuery .= "WHERE  $arrayColumna[$_column] = :value ORDER BY agen.id DESC LIMIT $_limite ";

            $mysqlStmt = $this->pdo->prepare($mysqlQuery);

            $mysqlStmt->bindParam(':value', $_id_vehiculo, PDO::PARAM_INT);
                ...
                if (intval($mysqlStmt->rowCount()) > 0) {
                    while ($row = $mysqlStmt->fetch(PDO::FETCH_BOTH)) {
                        array_push($mysqlArray, array(
                            'contador' => $count++,
                            'asistir' => getSpecialDate($row['fecha']),
                            'horario' => htmlspecialchars($row['nombre_horario']),
                            'canal' => htmlspecialchars($row['nombre_canal']),
                            'estado' => htmlspecialchars($row['nombre_estado']),
                            'fecha' => getSpecialDateTime($row['fecha_formulario']),
                            'responsable' => htmlspecialchars($row['responsable']),
                        ));
                    }
                    $this->arrayResponse = array(
                        'statusCode' => 200,
