import { QrCreate } from "./QrCreate.js";
export class PlacaClass {
  constructor(_ingreso) {
    ...
    this.btnQr = document.getElementById('btn-qr-generator');
  ...
  #funcListener(_class) {
    ...
    this.btnQr.addEventListener('click', (e) => {
      e.preventDefault();
      this.qrCreate.formularioSubmit();
    });
  ...
  datosSubmit() {
    ...
    let self = $.confirm({
      ...
      content: function () {
        return $.ajax({
          url: getMyAppModel('ingreso-completo/Ingreso', 'VehiculoInformacion'),
          type: 'POST',
