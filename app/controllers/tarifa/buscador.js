let validateFormEditarTarifa = $('#form_tarifa').validate({
  rules: {
    tipo_conexion: {
      required: true,
      range: [1, 4],
...
export const buscadorTarifa = new VisorBuscador(
  'form_0_buscador',
  'form_0',
  PROTOCOL_HOST +
    '/modulos/app/modelo/tarifa/tarifa.modelo.php?m=Listado',
...
$('#form_tarifa').on('reset', function (e) {
  validateFormEditarTarifa.resetForm();
});
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/app/modelo/tarifa/tarifa.modelo.php?m=Info',
        type: 'POST',
        data: {
          id: _id_element,
        },
        ...
        .done(function (response) {
          if (response.statusText === 'bien') {
            self.close();
            $('#form_id_tarifa').val(response.message['id']);
            $('#form_1_tarifa_desde').val(response.message['desde']);
            $('#form_1_tarifa_hasta').val(response.message['hasta']);
            $('#form_1_tarifa_precio').val(response.message['precio']);
            $('#form_1_tarifa_tipo_vehiculo').val(response.message['nombre']);
            $('#div_dialog_editar_tarifa').dialog('open');
          } else if (
...
$('#div_dialog_editar_tarifa, #form_tarifa').on(
  'submit',
  function (e) {
    if ($(this).valid() === true) {
      ...
      $.confirm({
        ...
        content: `
      <center>
      ¿Está seguro de alterar los datos de la tarifa?
      Los datos de su sesión serán tomados.
        ...
        buttons: {
          si: {
            text: 'Si',
            action: function () {
              funcGuardarTarifaSubmit(form_data);
            },
          },
          no: {
            text: 'No',
...
function funcGuardarTarifaSubmit(_form) {
  let self = $.confirm({
    title: false,
    content: 'Cargando, espere...',
    typeAnimated: true,
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/app/modelo/tarifa/tarifa.modelo.php?m=Update',
        type: 'POST',
        data: _form,
        processData: false,
        contentType: false,
        ...
        .done(function (response) {
          ...
          if (response.statusText === 'bien') {
            self.setTitle(response.statusText);
            self.setContent(response.message);
            $('#div_dialog_editar_tarifa').dialog('close');
            self.close(true);
          } else if (
            response.statusText === 'no_session' ||
            response.statusText === 'no_token'
  ...
  let self = $.confirm({
    ...
    content: function () {
      return $.ajax({
        url:
          PROTOCOL_HOST +
          '/modulos/app/modelo/tarifa/tarifa.modelo.php?m=Delete',
        type: 'POST',
        data: {
...
$('#div_dialog_editar_tarifa').dialog({
  width: 'auto',
  height: 435,
