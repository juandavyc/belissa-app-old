export class SmsClass {
  ...
  getNumeroClientes() {
    ...
    if (tel_1 === tel_2) {
      $('#container-numero-clientes').append(`  
      <input type="checkbox" id="sms-vehiculo" name="numero[]" value="${tel_1}"  DISABLED >
      <label for="sms-vehiculo"> <b>${$('#datos-propietario-nombre').val()}</b> ( ${tel_1} ) </label>
    `);
    } else {
      $('#container-numero-clientes').append(`  
      <input type="checkbox" id="sms-vehiculo-propietario" name="numero[]" value="${tel_1}"  DISABLED>
      <label for="sms-vehiculo-propietario"> <b>${$('#datos-propietario-nombre').val()}</b> ( ${tel_1} ) </label>
      <input type="checkbox" id="sms-vehiculo-conductor" name="numero[]" value="${tel_2}"  DISABLED>
  ...
  datosSubmit() {
    ...
    let self = $.confirm({
      ...
      content: function () {
        return $.ajax({
          url: PROTOCOL_HOST + '/modulos/app/modelo/callcenter/rtm.modelo.php?m=SendSms',
          type: 'POST',
          data: form_data,
          headers: {
          ...
          .done(function (response) {
            ...
            if (response.statusText === 'bien') {
              ...
              self.setTitle('Bien');
              self.setContent('Mensaje enviado');
            }
