export class RtmClass {
  constructor() {
    ...
    this.configuracionTabla = {
      botones: [],
      titulo: ['nro', 'cda', 'expedición', 'vencimiento', 'resultado', 'responsable'],
    };
  ...
  datosRecargar() {
    ...
    let self = $.confirm({
      ...
      content: function () {
        return $.ajax({
          url: getMyAppModel('callcenter/CallCenter', 'RtmListado'),
          type: 'POST',
          data: {
  ...
  datosSubmit() {
    ...
    let self = $.confirm({
      ...
      content: function () {
        return $.ajax({
          url: getMyAppModel('callcenter/CallCenter', 'RtmCreate'),
          type: 'POST',
  ...
  #autoCompleteInputs() {
    myAutocomplete({
      ...
      create: true,
      input: {
        text: document.getElementById("rtm-historial-cda-text"),
        hidden: document.getElementById("rtm-historial-cda-select"),
      },
      table: ['id', 'nombre', 'cda'],
      childs: [],
