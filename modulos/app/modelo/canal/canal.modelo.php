...
require $_SERVER["DOCUMENT_ROOT"] . '/modulos/app/clases/canal/canal.class.php';
// cambiar la comprobacion de la sesion para devolver el error
// var_dump($method);
$config = new RecursosApp();
$method = isset($_GET['m']) ? $_GET['m'] . 'canal' : 'error';
...
if (
    ...
    if ($database->estadoConexion()['status']) {
        ...
        $canalModelo = new CanalModelo($database->getPDO(), $config);
        // var_dump($method);
        $canalModelo->$method();
    } else {
...
class CanalModelo
{
    ...
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.create.php';        
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Listadocanal()
    {

        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.listado.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Deletecanal()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.eliminar.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Infocanal()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.informacion.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Updatecanal()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.actualizar.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }
    
    public function Tabcanal()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/canal/canal.tabla.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }
