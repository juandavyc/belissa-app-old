...
$method = isset($_GET['m']) ? $_GET['m'] . 'pdf' : 'error';
...
if (
    ...
    if ($database->estadoConexion()['status']) {
        $pdfModelo = new PdfModelo($database->getPDO(), $config);
...
class PdfModelo
{
    ...
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/visor-pdf/pdf.listado.php';
