...
require $_SERVER["DOCUMENT_ROOT"] . '/modulos/app/clases/archivo/archivo.class.php';
// cambiar la comprobacion de la sesion para devolver el error
// var_dump($method);
$config = new RecursosApp();
$method = isset($_GET['m']) ? $_GET['m'] . 'archivo' : 'error';
...
if (
    ...
    if ($database->estadoConexion()['status']) {
        ...
        $archivoModelo = new ArchivoModelo($database->getPDO(), $config);
        // var_dump($method);
        $archivoModelo->$method();
    } else {
...
class ArchivoModelo
{
    ...
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/archivo/archivo.create.php';
        
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Listadoarchivo()
    {

        require_once DOCUMENT_ROOT . '/modulos/app/modelo/archivo/archivo.listado.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }

    public function Deletearchivo()
    {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/archivo/archivo.eliminar.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }
    public  function Updatearchivo()
        {
        require_once DOCUMENT_ROOT . '/modulos/app/modelo/archivo/archivo.descargar.php';
        $this->configuracion->petResponse($this->arrayResponse);
    }
}